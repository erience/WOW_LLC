...
const PresaleForm = () => {
    ...
    const connectWallet = async () => {
        if (!window.ethereum) {
            const notify = () => toast('Please install MetaMask!', { position: 'top-right' });
            notify();
        ...
        try {
            ...
            const notify = () => toast('Failed to connect wallet!', { position: 'bottom-right' });
            notify();
    ...
    const getSmartContractData = async () => {
        try {
            ...
            const presaleRate = await initPresaleContract.methods.presaleRate().call();
    ...
    async function getWalletData() {
        try {
            ...
            const tokenContract = new web3Connect.eth.Contract(tokenABI, usdtContract);
    ...
    const swapforUSDT = async () => {
        try {
            if (!isConnected || !address) {
                ...
                const notify = () => toast('Please connect your wallet!', { position: 'bottom-right' });
                notify();
                return;
            }
            ...
            const usdtEquivalent = await initPresaleContract.methods.getReceivablePresaleTokens(usdtAmountFromInput, false).call();
            ...
            if (swapTxforUSDT.status) {
                const notify = () => toast('Presale Purchase transaction successful!', { position: 'bottom-right' });
                notify();
            } else {
                const notify = () => toast('Presale Purchase transaction failed!', { position: 'bottom-right' });
                notify();
            }

        } catch (error) {
            console.error("Error in buy function:", error);
            const notify = () => toast('An error occurred', { position: 'bottom-right' });
            notify();
        }
    }
    ...
    const swapforETH = async () => {
        try {
            ...
            const ethEquivalent = await initPresaleContract.methods.getReceivablePresaleTokens(ethAmountFromInput, false).call();
            ...
            const swapTxforETH = await initPresaleContract.methods
                .swapForETH()
                .send({ from: address, value: ethAmountFromInput.toString() });
        ...
        <div className={s.presaleBlock}>
            <form onSubmit={handleSubmit}>
                ...
                <div className={s.inputGroup}>
                    <label htmlFor="paymentAmount">{t('lable1')} {selectedCurrency === 'card' ? 'Fiat' : currencies.find(c => c.id === selectedCurrency)?.name}  {t('lable1-span')}</label>
                ...
                {typeof paymentAmount === "number" && paymentAmount > 0 && (
                    <div className={s.bottomButtonsGroup}>
                        <button
                            className={s.connect}
                            type="submit"
                            onClick={() => selectedCurrency === "eth" ? swapforETH() : swapforUSDT()}
                        >
                            {t("submitBtn")}
                        </button>
                    </div>
